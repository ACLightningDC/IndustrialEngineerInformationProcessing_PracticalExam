...
INSERT INTO coffee_tbl values('A005','감자튀김', 5000);

create table coffe_tbl_
...
select pname , pcost , sum(amount) as amount , nvl(pcost*sum(amount), 0)as totalSum from sale_tbl sale full join coffee_tbl coffe on sale.pcode = coffe.pcode group by pname , pcost union  select '아이스 아메리카노' , 3500 , 0 , pcost*sum(amount) from dual order by totalSum desc; 
select pname , pcost , nvl(sum(amount) , 0)as amount , nvl(pcost*sum(amount),0) as totalSum from sale_tbl sale full join coffee_tbl coffe on sale.pcode = coffe.pcode group by pname , pcost order by TOTALSUM desc; 
...
from on using 
--slect2_2.jsp
select C1.PCODE , C1.PNAME , C2.PCOST
from coffee_tbl1 C1 , coffee_tbl2 C2
where C1.PCODE = C2.PCODE
AND C1.PCOst != C2.PCOST
