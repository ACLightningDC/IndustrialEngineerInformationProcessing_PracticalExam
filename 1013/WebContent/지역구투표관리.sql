...
DROP SEQUENCE TBL_TEACHER_202201_seq;
...
substr(m_jumin , '1','6') ||'-'||substr(m_jumin , '6') ,
m_city,
p_tel1||'-'||p_tel2||'-'||p_tel3
from tbl_party_202005 natural join tbl_member_202005 ;

--selectVote.jsp
select v_name ,
'19' || substr(v_jumin ,'1','2')||'년'||substr(v_jumin ,'3','2')|| '월'||substr(v_jumin ,'5','2')||'일생',
decode(substr(v_jumin,'7','1'),'1','남자','2','여자'),
m_no,
substr(v_time, '1','2')||':'||substr(v_time, '3','4'),
decode(v_confirm ,'Y','확인','N','미확인')
from tbl_vote_202005;

--voteMemberResult
select member.m_no, member.m_name, count(vote.m_no) as result
from tbl_member_202005 member join tbl_vote_202005 vote

group by member.m_no, member.m_name
